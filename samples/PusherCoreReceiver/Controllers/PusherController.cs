...
namespace PusherCoreReceiver.Controllers
{
    public class PusherController : ControllerBase
    {
        ...
        [PusherWebHook(Id = "It")]
        public IActionResult PusherForIt(string[] events, PusherNotifications data)
        {
        ...
        [PusherWebHook]
        public IActionResult Pusher(string id, string[] events, JObject data)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            var createdAt = DateTimeOffset.FromUnixTimeMilliseconds(createdAtUnix);
            var eventArray = data.Value<JArray>(PusherConstants.EventRequestPropertyContainerName);
            _logger.LogInformation(
                5,
            ...
            // Get details of the individual notifications.
            var eventEnumerable = eventArray.Values<JObject>();
            var index = 0;
