...
//ToDo - All test should be re-written after correct implementation of ASP.NET Webhooks for BitBucket
namespace Microsoft.AspNet.WebHooks
{
    /*
    public class BitBucketWebHookReceiverTests
    {
        ...
        private Mock<BitBucketWebHookReceiver> _receiverMock;
        private HttpRequestMessage _postRequest;

        public BitBucketWebHookReceiverTests()
        {
            _settings = new SettingsDictionary();
            _settings["MS_WebHookReceiverSecret_BitBucket"] = TestSecret;
            ...
            _receiverMock = new Mock<BitBucketWebHookReceiver> { CallBase = true };
        ...
        {
            // Arrange
            _postRequest.Headers.Add(BitBucketWebHookReceiver.SignatureHeaderName, "value1");
            _postRequest.Headers.Add(BitBucketWebHookReceiver.SignatureHeaderName, "value2");
        ...
        {
            // Arrange
            _postRequest.Headers.Add(BitBucketWebHookReceiver.SignatureHeaderName, header);
        ...
        {
            // Arrange
            _postRequest.Headers.Add(BitBucketWebHookReceiver.SignatureHeaderName, "sha1=invalid");
        ...
        public async Task ReceiveAsync_Throws_IfPostHasInvalidSignature()
        {
            ...
            // Assert
            HttpError error = await ex.Response.Content.ReadAsAsync<HttpError>();
            Assert.Equal("The WebHook signature provided by the 'X-Hub-Signature' header field does not match the value expected by the 'BitBucketWebHookReceiverProxy' receiver. WebHook request is invalid.", error.Message);
            _receiverMock.Protected()
                .Verify<Task<HttpResponseMessage>>("ExecuteWebHookAsync", Times.Never(), TestReceiver, _context, _postRequest, ItExpr.IsAny<IEnumerable<string>>(), ItExpr.IsAny<object>());
        }
        ...
        public async Task ReceiveAsync_ReturnsError_IfPostHasNoEventHeader(string header)
        {
            ...
            Assert.Equal("The WebHook request must contain a 'X-BitBucket-Event' HTTP header indicating the type of event.", error.Message);
            _receiverMock.Protected()
                .Verify<Task<HttpResponseMessage>>("ExecuteWebHookAsync", Times.Never(), TestReceiver, _context, _postRequest, ItExpr.IsAny<IEnumerable<string>>(), ItExpr.IsAny<object>());
        }
        ...
        public async Task ReceiveAsync_Succeeds_IfValidPostRequest(string header)
        {
            ...
            _postRequest.Headers.Add(BitBucketWebHookReceiver.SignatureHeaderName, header);
            _postRequest.Headers.Add(BitBucketWebHookReceiver.EventHeaderName, "action1");
            _receiverMock.Protected()
        ...
        public async Task ReceiveAsync_Succeeds_IfPostPing(string header)
        {
            // Arrange
            _postRequest.Headers.Add(BitBucketWebHookReceiver.SignatureHeaderName, header);
            _postRequest.Headers.Add(BitBucketWebHookReceiver.EventHeaderName, BitBucketWebHookReceiver.PingEvent);
