...
namespace Microsoft.AspNetCore.WebHooks
{
    ...
    public class WebHookManager : IWebHookManager, IDisposable
    {
        ...
        protected virtual void VerifyUri(Uri webHookUri)
        {
            ...
            {
                var message = $"The WebHook URI must be absolute with a scheme of either 'http' or 'https' but received '{webHookUri}'.";
                _logger.LogError(message);
                throw new InvalidOperationException(message);
            }
        }
        ...
        protected virtual void VerifySecret(string secret)
        {
            ...
            {
                throw new InvalidOperationException("The WebHook secret key parameter must be between 32 and 64 characters long.");
            }
        }
    }
}
